        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col lg:flex-row gap-6 h-auto lg:h-96">
                ...
                <div class="w-full lg:w-1/4 bg-gradient-to-br from-uin-yellow/20 via-white to-uin-green/20 dark:from-gray-800 dark:to-gray-900 border border-uin-yellow/30 rounded-xl p-8 flex flex-col justify-center items-start relative overflow-hidden group">
                    ...
                    <a href="{{ route('books.index') }}" class="relative z-10 inline-flex items-center justify-center px-6 py-3 text-sm font-bold text-white transition-all duration-200 bg-gray-900 dark:bg-uin-blue rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 hover:bg-gray-800 dark:hover:bg-blue-800">
                        {{ __('messages.top.view_all') }}
                ...
                <div class="w-full lg:w-3/4 flex gap-4 overflow-x-auto pb-4 snap-x hide-scrollbar scroll-smooth">
                    ...
                    @forelse($latestBooks as $index => $book)
                        <a href="{{ route('books.read', $book->id) }}" class="snap-center shrink-0 w-64 h-full bg-gray-50 dark:bg-gray-800 rounded-xl p-6 flex flex-col items-center justify-between hover:shadow-lg transition border border-transparent hover:border-uin-blue/20 cursor-pointer group">
                            
                            <div class="w-32 h-48 bg-gray-200 dark:bg-gray-700 rounded shadow-md mb-4 overflow-hidden relative transition transform group-hover:scale-105">
                                @if($book->cover_path)
                                    <img src="{{ asset('storage/' . $book->cover_path) }}" alt="{{ $book->title }}" class="w-full h-full object-cover">
                                @else
                                    <div class="w-full h-full flex items-center justify-center text-gray-400 text-xs font-bold uppercase tracking-widest text-center p-2">
                                        No Cover
                                    </div>
                                @endif
                                <div class="absolute top-2 right-2 bg-white/90 dark:bg-gray-900/90 backdrop-blur px-2 py-0.5 rounded text-[10px] font-bold text-uin-blue dark:text-uin-yellow shadow-sm">
                                    {{ $book->category }}
                                </div>
                            </div>

                            <div class="text-center mb-4 w-full">
                                <h3 class="font-bold text-gray-900 dark:text-white text-lg leading-tight line-clamp-2" title="{{ $book->title }}">
                                    {{ $book->title }}
                                </h3>
                                <p class="text-xs text-gray-500 mt-1 truncate">{{ $book->author }}</p>
                            </div>

                            <div class="w-10 h-10 rounded-full border-2 border-gray-900 dark:border-white flex items-center justify-center text-xl font-bold text-gray-900 dark:text-white group-hover:bg-gray-900 group-hover:text-white dark:group-hover:bg-white dark:group-hover:text-gray-900 transition-colors">
                                {{ $index + 1 }}
                            </div>
                        </a>
                    @empty
                        <div class="w-full flex flex-col items-center justify-center text-gray-400 py-10 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                    ...
                    <div class="snap-center shrink-0 w-32 h-full flex flex-col items-center justify-center">
                        <a href="{{ route('books.index') }}" class="w-14 h-14 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 transition group">
                            <svg class="w-6 h-6 text-gray-600 dark:text-gray-300 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                        </a>
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-3 text-center">{{ __('messages.top.view_all') }}</span>
                    </div>
                    @endif
